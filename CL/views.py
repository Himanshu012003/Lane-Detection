...
def undistort_img():
    ...
    pickle.dump( dist_pickle, open('media/cal_pickle.p', 'wb') )

def undistort(img, cal_dir='media/cal_pickle.p'):
...
def runcode(request):
    ...
    # if fs.exists("input.mp4"):
    #     fs.delete("input.mp4")
    # if fs.exists("output.mp4"):
    #     fs.delete("output.mp4")
    ...
    # im.save("media/new_image.jpg")




    filename = fs.save(inpu+".mp4", data[0] )
    myclip = VideoFileClip('media/'+inpu+'.mp4')#.subclip(40,43)
    output_vid = 'media/'+output+'.mp4'
    clip = myclip.fl_image(vid_pipeline)
    clip.write_videofile(output_vid, audio=False)   
    return render(request,"front.html",{"video":"/media/"+inpu+".mp4","new_video":"/media/"+output+".mp4","flag":1})
def home(request):     
    return render(request,"front.html",{"flag":0})
